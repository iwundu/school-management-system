...
if(isset($_SESSION['uname']) == false){
    ...
    $result = mysqli_query($conn,"SELECT * FROM teachers WHERE fname = '$loggedin_user' LIMIT 1") or die(mysqli_error($conn));
...
if($error != ""){
    $error .= '<div class="alert alert-danger" role="alert"> Please fill all fields!'.$error.'
        ...
        $sql = "INSERT INTO result(student_name,class,subject,1st_CAT,2nd_CAT,3rd_CAT,exam,average_score,total_score,grade) VALUES('$sname','$class','$subject','$first_cat','$second_cat','$third_cat','$exam','$ascore','$tscore','$grade')";
        $Rresult = mysqli_query($conn,$sql) or die(mysqli_error($conn));
        
    $error .= '<div class="alert alert-success" role="alert"> Result Uploaded Successfully!
</div>';
}
